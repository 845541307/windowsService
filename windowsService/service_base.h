...
class ServiceBase {
 ...
 protected:
	 ServiceBase(const CString& name,
		 ...
		 bool  bHangUpMode= false,
         const CString& depends = _T(""),
         const CString& account = _T(""),
         const CString& password = _T(""));

  void SetStatus(DWORD dwState, DWORD dwErrCode = NO_ERROR, DWORD dwWait = 0, DWORD dwSpecificExitCode = 0);
  ...
  virtual bool OnInit(DWORD argc, TCHAR* argv[]) = 0;
  virtual void OnStart() {}
  virtual void OnStop() {}
  virtual void OnPause() {}
  virtual void OnContinue() {}
  virtual void OnShutdown() {}
  virtual void OnCustomHandle(DWORD eventId) {}
 ...
 private:
  ...
  bool Init(DWORD argc, TCHAR* argv[]);
  void Start();
  void Stop();
  void Pause();
  void Continue();
  ...
  // About whether the service main function suspends mode
  bool m_IsHandUpMode;
